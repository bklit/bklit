...
Create or update your `.env` file in the project root with the following structure:
...
# ===================
# Keep these for production deploys
DATABASE_URL="your-production-postgres-url"
CLICKHOUSE_HOST="your-production-clickhouse-url"
...
- `pnpm db:migrate` → Uses `DEV_DATABASE_URL`
- `pnpm db:studio` → Connects to local database
- `pnpm run migrate` (in analytics package) → Uses `DEV_CLICKHOUSE_*` variables
