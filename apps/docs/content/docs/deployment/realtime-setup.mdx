...
1. **SDK** - Opens WebSocket to `wss://bklit.ws`
2. **WebSocket Server** - Receives events, queues to Redis, broadcasts to dashboards
3. **Worker** - Processes queue, saves to ClickHouse
4. **Dashboard** - Connects to WebSocket for instant updates

**Key Feature:** Sessions end **instantly** (under 1 second) when visitors close browser tabs via WebSocket disconnect detection.
...
Proxy: DNS only (gray cloud in Cloudflare) ⚪ IMPORTANT!
TTL: Auto
```
...
That's it! Your analytics are now real-time with sub-second updates. 🚀
